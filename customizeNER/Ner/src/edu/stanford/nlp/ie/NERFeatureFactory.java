...
//    log.info(doFE+"\t"+domain);
  ...
  protected void featuresC(PaddedList<IN> cInfo, int loc, FeatureCollector out) {
    ...
    if (flags.useInternal && flags.useExternal ) {
      ...
      //add by yue
      if (flags.useConsitiuencyHead) {
    	  String head_pos = c.getString(CoreAnnotations.ConsitituencyHeadAnnotation.class);
    	  if (!head_pos.isEmpty()) {
    		  out.build().append(head_pos).append("-CONSITUHEAD").add();
    	  }
      ...
      //add by yue
      if (flags.useConsitiuencyRight) {
    	  String right_pos = c.getString(CoreAnnotations.ConsitituencyRightAnnotation.class);
    	  if (!right_pos.isEmpty()) {
    		  out.build().append(right_pos).append("-CONSITURIGHT").add();
    	  }         
        }
      
      //add by yue
      if (flags.useConsitiuencyLeft) {
    	  String left_pos = c.getString(CoreAnnotations.ConsitituencyLeftAnnotation.class);
    	  if (!left_pos.isEmpty()) {
    		  out.build().append(left_pos).append("-CONSITULEFT").add();
    	  }         
        }
      
      //add by yue
      if (flags.useConsitiuencyPosi) {
    	  String posi = c.getString(CoreAnnotations.ConsitituencyPosiAnnotation.class);
    	  if (!posi.isEmpty()) {
    		  out.build().append(posi).append("-CONSITUPOSI").add();
    	  }         
        }
